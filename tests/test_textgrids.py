from fave_recode.textgrid_process import processNames
import pytest
...
class TestTierNameProcess:
    def test_all_two(self):
        splits = [processNames.splitTierInfo(tiername_dicts[x]) for x in tiername_dicts]
        split_len = [len(x) for x in splits]
        split_is_two = [x == 2 for x in split_len]
        assert all(split_is_two)

    def test_no_phone_word(self):
        no_phone_word_split = processNames.splitTierInfo(tiername_dicts["no_phone_word"])
    ...
    def test_second_is_wp(self):
        ...
        splits = [processNames.splitTierInfo(tiername_dicts[x]) for x in wp_keys]
